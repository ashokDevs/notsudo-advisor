...
async def test_reason_node_no_context_concludes_safe() -> None:
    ...
    assert result["reachability_reasoning"] == "No call sites found."
...
async def test_act_node_exposed_creates_pr_draft() -> None:
    state = _base_state(is_exposed=True, reachability_reasoning="merge() called in utils.js")
    ...
    assert result["pr_draft"]["body"] == "merge() called in utils.js"
...
def test_route_reason_needs_more_context_loops_back() -> None:
    state = _base_state(retrieval_iterations=1, reachability_reasoning="Need more context")
    assert route_reason(state) == "locate"


def test_route_reason_default_proceeds_to_act() -> None:
    state = _base_state(retrieval_iterations=0, reachability_reasoning="Conclusive.")
